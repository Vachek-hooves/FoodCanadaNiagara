...
const RecipeDetailsCard = ({route}) => {
  ...
  const removeNotes = async () => {
    ...
    const json = await AsyncStorage.getItem('myRecipeDeleted');
    ...
    await AsyncStorage.setItem(
      'myRecipeDeleted',
      JSON.stringify(recipeDeleted),
    );

    await AsyncStorage.setItem('myRecipe', JSON.stringify(filtered));
    console.log('remove');
  };
    ...
    <Layout>
      ...
      <View style={styles.footer}>
        <View style={{marginHorizontal: 16, flexDirection: 'row', gap: 16}}>
          ...
          <TouchableOpacity
            onPress={() => removeNoteNotification()}
            activeOpacity={0.7}
            style={{
              width: '48%',
              height: 56,
